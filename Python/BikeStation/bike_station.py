...
class BikeStation:
    def __init__(self, bike_station):
        if isinstance(bike_station, tuple):     # To be called with Tuple object in case of data fetched from SQLite
            self.record = bike_station[0]
            self.id = bike_station[1]
            self.station_name = bike_station[2]
            self.total_docks = bike_station[3]
            self.timestamp = bike_station[4]
            self.available_docks = bike_station[5]
            self.docks_in_service = bike_station[6]
        else:
    ...
    def as_dict(self):
        """
        To convert class object to dictionary for pandas dataframe.
        :return: dict
        """
        return {'id': self.id
                ...
                , 'docks_in_service': self.docks_in_service
    ...
    def __str__(self):
        timestamp_str = self.timestamp.strftime("%a %b %d %X %Y")

/*  
  i) Food Delivery Application, ISMG 6080 and Database Management Systems E01, Spring 2021
 ii) Group # - Project 13; Name - Sribalaji Thirunavukkarasu,
iii) Oracle account user Id - ORA49
 iv) My project tables reside in the cloud database
  v) Below is the list of the implemented tables in my Oracle account only (Addl. 3 tables can be found in Accdb file). 
  
TABLE Customer; PK = CustID  
 CustID
 FName	  
 Lname	  
 Address  
...
TABLE PaymentType; PK = TypeId
 TypeId               
 Description         
 
TABLE Restaurant; PK = RID
 RID	 
 RPhone	  
 Raddress  
 Rlocation  
...
TABLE MenuItem;  PK = MID; FK = RestuarantId (Restaurant  RID)
 MID               
 Description                 
 UnitePrice                 
 RestuarantId                
 
TABLE DeliveryAgent;  PK = AgentId
	AgentId	
	Aname	
	Vehicle#	
	Aphone	
 
TABLE OrderStatus; PK = StatusId
	StatusId             
	Description         
	
TABLE Payment;  PK = PID; FK = TypeId (PaymentType  TypeId)
	PID             
	Status             
	PDate DATE
	TypeId  
	
TABLE Order; PK = OrderId; FK = CustId (Customer  CustID), RID (Restaurant  RID), PID (Payment  PID), AgentId (DeliveryAgent  AgentId)	
	OrderId	
	ODate DATE
	TotalAmount	
	CustId	
...
TABLE OrderItem; PK = CompositeKey(OrderId, MID); FK = OrderId (Order  OrderId), MID (MenuItem  MID)	
	MID	
	OrderId	
	CurrentPrice	
	Qty 
		
TABLE OrderFeedback; PK = FID; FK = OrderId (Order  OrderId)
	FID	
	Feedback 
	Rating 	
	OrderId		
...
/* Query 1 - To find all the menu Items provide by Udom Thai Restuarant */
select * from "MenuItem" inner join "Restaurant" on "MenuItem".RESTUARANTID = "Restaurant".RID where "Restaurant".RNAME = 'Udom Thai';

/* Query 2 - To get the types of payment for each order */
select orderid, "PaymentType".DESCRIPTION from "Order" inner join "Payment" on "Order".PID = "Payment".PID inner join "PaymentType" on "Payment".TYPEID = "PaymentType".TypeId; 

/* Query 3 - To get total amount for orders from each restuarant */
select rid, sum(totalamount) from "Order" group by rid;

/* Query 4 - To get average order total amount for each restuarant */
select rid, ROUND(avg(totalamount),2) from "Order" group by rid;

/* Query 5 - To get total # of orders placed by each customer */
select custid, count(orderid) AS Total_orders from "Order" group by custid;

/* Query 6 - To get customer ID who placed orders using credit card */
select orderid, custid from "Order" inner join "Payment" on "Order".PID = "Payment".PID inner join "PaymentType" on "Payment".TYPEID = "PaymentType".TypeId where  "PaymentType".TypeId in ( select TypeID from "PaymentType" where DESCRIPTION = 'Credit Card');

/* Query 7 - To get customers who ordered Chicken King Roll */
select custid from "Order" inner join "OrderItem" on "Order".ORDERID = "OrderItem".OrderId where "OrderItem".MID = (select mid from "MenuItem" where description = 'Chicken King Roll');

/* Query 8 - To get all details for orders placed in the year 2020 and delivered by Agent Michael */
select * from "Order" where (ODATE between '01-Jan-2020' and '31-Dec-2020') and AGENTID = (select AgentID from "DeliveryAgent" where "DeliveryAgent".ANAME = 'Michael');

/* Query 9 - To get count of orders delivered by each delivery agent, ordered by deliveries */
select "Order".AGENTID, "DeliveryAgent".Aname, count("Order".ORDERID) AS deliveries_made from "Order" inner join "DeliveryAgent" on "Order".AGENTID = "DeliveryAgent".AgentID group by "Order".AGENTID, "DeliveryAgent".Aname order by count("Order".ORDERID) desc;

/* Query 10 - To get count of order made for each payment Type */
select "PaymentType".TYPEID, count("PaymentType".TYPEID) Total from "Order" inner join "Payment" on "Order".PID = "Payment".PID inner join "PaymentType" on "Payment".TYPEID = "PaymentType".TYPEID group by "PaymentType".TYPEID